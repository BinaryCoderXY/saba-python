...
bool AppContext::Setup()
{
    ...
    m_resourceDir = "/Users/apple/pysaba/build/saba/resource";
...
public:
    ...
    bool init(bool enableTransparentWindow)
    {
        if (!glfwInit())
        {
            return false;
        ...
        window = glfwCreateWindow(1280, 800, "simple mmd viewer", nullptr, nullptr);
        if (window == nullptr)
        {
    ...
    bool run()
    {
        glfwPollEvents(); // 👈 必须放在最开头！让系统先处理关闭事件
        double fpsTime = saba::GetTime();
        int fpsFrame = 0;
        double saveTime = saba::GetTime();
        while (!glfwWindowShouldClose(window))
...
PYBIND11_MODULE(mmd, m)
{
    ...
    py::class_<MMDViewer>(m, "MMDViewer")
        ...
        .def("run", &MMDViewer::run);
    py::class_<MMDShader>(m, "MMDShader")
        .def(py::init<>());
}