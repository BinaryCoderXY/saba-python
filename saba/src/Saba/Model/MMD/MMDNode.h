...
namespace saba
{
	...
	public:
		...
		bool IsManualBlend() const { return m_manualBlendEnable; }
		void SetManualBlendEnable(bool enable) { m_manualBlendEnable = enable; }

		// float GetManualBlendWeight() const { return m_manualBlendWeight; }
		void SetManualBlendWeight(float weight) { m_manualBlendWeight = glm::clamp(weight, 0.0f, 1.0f); }

		void SetManualAnimationRotate(const glm::quat &q, float blend_weight)
		{
			m_blendWeightRotation = glm::clamp(blend_weight, 0.0f, 1.0f);
			m_targetRotation = q;
		}
		void SetManualAnimationTranslate(const glm::vec3 &q, float blend_weight)
		{
			m_blendWeightTranslate = glm::clamp(blend_weight, 0.0f, 1.0f);
			m_targetTranslate = q;
		}
	...
	public:
		...
		glm::quat m_vmdAnimRotate;
		glm::vec3 m_vmdAnimTranslate;
		glm::quat m_targetRotation;
		glm::vec3 m_targetTranslate;
		float m_blendWeightRotation = 0.0f;
		float m_blendWeightTranslate = 0.0f;

	private:
		bool m_manualControl = false;
		float m_manualBlendWeight = 0.0f;
		// glm::quat m_manualRotate = glm::quat(1, 0, 0, 0);
		bool m_manualBlendEnable = false;
	};
