...
namespace saba
{
	...
	void VMDAnimation::Evaluate(float t, float weight)
	{
		for (auto &nodeCtrl : m_nodeControllers)
		{
			MMDNode *node = nodeCtrl->GetNode();
			if (node)
			{
				nodeCtrl->Evaluate(t, weight);
				node->m_vmdAnimRotate = node->m_animRotate;
				node->m_vmdAnimTranslate = node->m_animTranslate;
				if (node->IsManualControl())
				{
					node->m_animRotate = glm::slerp(
						node->m_vmdAnimRotate,
						node->m_targetRotation,
						node->m_blendWeightRotation);
					node->m_animTranslate = glm::mix(
						node->m_vmdAnimTranslate,
						node->m_targetTranslate,
						node->m_blendWeightTranslate);
					continue;
				}
			}
		}
		...
		for (auto &morphCtrl : m_morphControllers)
		{
			MMDMorph *morph = morphCtrl->GetMorph();
			if (morph)
			{
				if (morph->IsManualControl())
				{
					morph->m_weight = glm::mix(
						morph->m_weight_vmd,	// 原始VMD表情
						morph->m_target_weight, // 手动表情
						morph->blend_weight);	// 混合权重);
					continue;
				}
			}
